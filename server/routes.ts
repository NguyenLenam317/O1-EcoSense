...
import { getUserFromSession, type AuthenticatedRequest } from "./auth/session";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Chat API Authentication Middleware
  ...
  app.post("/api/chat/message", async (req: Request, res: Response) => {
    try {
      ...
      // Rate limiting - implement a more robust solution in production
      // This is a simple demonstration
      const rateLimitKey = `chat_rate_limit:${userId}`;
      const rateLimitData = req.app.locals[rateLimitKey] || { count: 0, timestamp: Date.now() };
      
      // Reset rate limit after 1 minute
      if (Date.now() - rateLimitData.timestamp > 60000) {
      ...
      while (retries > 0) {
        try {
          groqResponse = await getGroqCompletion(systemPrompt, chatHistory.messages);
          break;
        } catch (error) {
          retries--;
          console.error(`Error getting AI response. Retries left: ${retries}`, error);
          
          if (retries === 0) {
            return res.status(503).json({ message: "Unable to get AI response after multiple attempts. Please try again later." });
          }
